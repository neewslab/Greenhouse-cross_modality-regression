# -*- coding: utf-8 -*-
"""
Created on Wed Feb  7 19:51:53 2024
...
month_val = np.reshape(np.array([x.replace('/','') for x in month]).astype(np.float),(-1,1))



# data_con = np.concatenate((data_temp,data_voltage,data_PAR,data_light,time_val, month_val),1)

data_con = np.concatenate((data_temp,data_voltage,data_PAR,time_val, month_val),1)
...
history = model.fit(X_train, y_train, validation_data=(X_test, y_test), epochs=num_epochs, batch_size=128, verbose=2, callbacks=[es])
...
X__,y__ =data_con[70000:75000, 1:5], data_con[70000:75000, 0:1]
...
pred_temp = model.predict(data_con[:,1:5])

dataset_new = np.append(dataset,pred_temp, axis =1)
